...
export const BarGraph = ({ initialData }: {initialData?} )=> {
    ...
    const [dropdownValue, setDropdownValue] = React.useState(BarGraphOptions[0].value);
    ...
    function handleGenreFilter(genre: GenreResult) {
        ...
        const genreIndex = currentGenreArray.findIndex(g => g.genre === genre.genre);
        ...
        if (genreIndex !== -1) {
          ...
          setTimeout(() => {
            const newArray = updatedGenreArray.filter((_, index) => index !== genreIndex);
          ...
          updateGenreArray(genre.genre_type, [...currentGenreArray, { genre: genre.genre, genre_type: genre.genre_type }]);
        }
      }
      ...
      const fetchData = async () => {
        ...
        const response = await fetch(`api/aggregate?query=${JSON.stringify({
            by: [dropdownValue],
            _sum: {
              hours_played: true,
            },
            ...
            where: {
              AND: [
                ...
                {
                  OR: prismaGenreFilters({main_genre: getGenres(mainGenre), secondary_genre: getGenres(secondaryGenre)})
                }
              ]
            },
            take: 10,
    ...
    const option = BarGraphOptions.find(option => option.value === dropdownValue);
    ...
    return (
        ...
        <div className="flex flex-col gap-5 lg:gap-0 lg:flex-row justify-between items-center"> 
          ...
          <div className="flex flex-col lg:flex-row gap-5">
            <GenreBadges mainGenre={mainGenre} secondaryGenre={secondaryGenre} onBadgeClick={handleGenreFilter} />
            <div className="flex flex-row gap-5 order-1 lg:order-2">
            <GenreSearch onGenreSelect={handleGenreFilter} />
            <GraphDropDown options={BarGraphOptions} value={dropdownValue} onValueChange={setDropdownValue}/>
            </div>
          </div>
          </div>
        </CardHeader>             
